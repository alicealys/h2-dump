...
main()
{
    ...
    self thread [[ var_0 ]]();
    self._id_B597 = undefined;
    self._id_B30B = undefined;
    moveinit();
    getupifprone();
    ...
    if ( animscripts\stairs_utility::_id_BD37() )
        thread animscripts\stairs_utility::_id_BDFD();

    thread animdodgeobstaclelistener();
    var_3 = animscripts\utility::_id_C0F1();
...
pathchangelistener()
{
    ...
    if ( isdefined( level._id_BFAE ) && !level._id_BFAE )
        return;

    self._id_D479 = 0;
    self._id_BEE7 = getdvarint( "ai_turnAnim_handleEarlyNotifies", 1 ) == 0;
    self._id_04CD = 0;
    ...
    for (;;)
    {
        ...
        if ( self.facemotion )
        {
            if ( self getanimweight( %combatrun_backward ) > 0.1 )
            {
                self notify( "stop_move_anim_update" );
                self.update_move_anim_type = undefined;
                self.moveloopoverridefunc = ::_id_C22E;
                self notify( "move_loop_restart" );
                animscripts\run::endfaceenemyaimtracking();
            }
        }
...
{
    var_0 = isdefined( self._id_B0BB ) && self._id_B0BB;

    if ( isdefined( self._id_D28A ) && isdefined( self._id_BBFC ) )
    {
        var_1 = self._id_D28A - self._id_BBFC;

        if ( var_1 > 0 )
        {
            var_2 = gettime();
            var_3 = var_2 - self._id_D28A;
            var_4 = var_3 / var_1;
            var_0 = common_scripts\utility::ter_op( var_4 > 0.75, !self._id_B0BB, self._id_B0BB );
        }
    }
...
_id_CAC5()
{
    ...
    else if ( isdefined( self._id_CA31 ) && self._id_CA31 && maps\_patrol::_id_C8DD() )
    {
        ...
        if ( var_2 )
        {
            var_1[1] = level.scr_anim["generic"]["patrol_turn_l135_rfoot" + var_3];
            var_1[2] = level.scr_anim["generic"]["patrol_turn_l90_rfoot" + var_3];
            var_1[3] = level.scr_anim["generic"]["patrol_turn_l45_rfoot" + var_3];
            var_1[5] = level.scr_anim["generic"]["patrol_turn_r45_rfoot" + var_3];
            var_1[6] = level.scr_anim["generic"]["patrol_turn_r90_rfoot" + var_3];
            var_1[7] = level.scr_anim["generic"]["patrol_turn_r135_rfoot" + var_3];
        }
        else
        {
            var_1[1] = level.scr_anim["generic"]["patrol_turn_l135_lfoot" + var_3];
            var_1[2] = level.scr_anim["generic"]["patrol_turn_l90_lfoot" + var_3];
            var_1[3] = level.scr_anim["generic"]["patrol_turn_l45_lfoot" + var_3];
            var_1[5] = level.scr_anim["generic"]["patrol_turn_r45_lfoot" + var_3];
            var_1[6] = level.scr_anim["generic"]["patrol_turn_r90_lfoot" + var_3];
            var_1[7] = level.scr_anim["generic"]["patrol_turn_r135_lfoot" + var_3];
        }
    ...
    else if ( self.movemode == "walk" )
    {
        ...
        var_1 = animscripts\utility::lookupanimarray( var_4 );
    }
    else if ( isdefined( self.animarchetype ) && self.animarchetype == "riotshield" )
    {
...
pathchange_getturnanim_impl( var_0, var_1 )
{
    ...
    if ( animscripts\utility::_id_BBC3() )
        var_7 = 22.5;
    else
        var_7 = 10;

    if ( var_6 )
        var_8 = animscripts\utility::_id_D183();
    else
...
pathchange_candoturnanim( var_0 )
{
    ...
    var_8 = !animscripts\utility::_id_BBC3() && getdvarint( "ai_turnAnim_checkFullTrace", 0 );
...
pathchange_doturnanim()
{
    ...
    var_2 = isdefined( self._id_CA31 ) && self._id_CA31 && maps\_patrol::_id_C8DD();
    ...
    if ( isdefined( self.casual_killer ) && self.casual_killer && self.moveplaybackrate > 1 )
        var_3 = 1 + 0.3 * ( self.moveplaybackrate - 1 );
    ...
    maps\_utility::delaychildthread( var_5, animscripts\stairs_utility::_id_C547, var_0, 0, var_4, undefined, "inOnly" );
...
_id_C22E()
{
    ...
    if ( isdefined( self.animarchetype ) && self.animarchetype == "riotshield" )
        var_0 = animscripts\utility::lookupanim( "walkBack2Run", 8 );
    else
        var_0 = %h2_walk_backward_trans_2_run_2;

    self animmode( "zonly_physics", 0 );
    self clearanim( %body, 0.1 );
    self.moveloopcleanupfunc = ::_id_BD0F;
    self.ignorepathchange = 1;
    self setflaggedanimrestart( "facemotion_reorient_anim", var_0, 1, 0.05, self.moveplaybackrate );
    self orientmode( "face angle", self.angles[1] );
    animscripts\shared::donotetracks( "facemotion_reorient_anim" );
    self.ignorepathchange = undefined;
    self orientmode( "face motion" );
    self animmode( "none", 0 );
    animscripts\shared::donotetracks( "facemotion_reorient_anim" );
...
dodgemoveloopoverride()
{
    ...
    if ( isdefined( self.currentdodgeanim ) && animhasnotetrack( self.currentdodgeanim, "code_move" ) )
        animscripts\shared::donotetracks( "dodgeAnim" );
    ...
    for (;;)
    {
        ...
        if ( animscripts\utility::isincombat() )
        {
            if ( self.type == "civilian" )
            {
                ...
                var_2 = self.origin + ( 0.0, 0.0, 15.0 );
        ...
        if ( self.type == "civilian" && animscripts\utility::isincombat() )
            self._id_B490 = 1;
...
setdodgeanims( var_0, var_1 )
{
    ...
    self.dodgeleftanimoffset = getangledelta( var_0, 0, var_2 );
    var_2 = 1;

    if ( animhasnotetrack( var_1, "code_move" ) )
        var_2 = getnotetracktimes( var_1, "code_move" )[0];

    self.dodgerightanimoffset = getangledelta( var_1, 0, var_2 );
    self.interval = 80;
}
...
_id_B799()
{
    return !isdefined( self._id_C032 ) || gettime() - self._id_C032 > getdvarint( "ai_reactToBulletCooldown" );
}
...
bulletwhizbycheck_whilemoving()
{
    ...
    for (;;)
    {
        ...
        if ( self.lookaheadhitsstairs || self.lookaheaddist < 100 || self.lookaheaddist < anim._id_B488 )
            continue;
        ...
        self._id_C032 = self.requestreacttobullet;
        self notify( "move_loop_restart" );
        animscripts\run::endfaceenemyaimtracking();
    }
}
...
movestand_moveoverride( var_0, var_1 )
{
    ...
    var_3 = animscripts\utility::_id_B8C3();
    ...
    else
    {
        ...
        if ( var_2 )
        {
            var_5 = animscripts\stairs_utility::_id_D210();
        ...
        if ( !isdefined( var_4 ) )
        {
            if ( isarray( var_0 ) )
            {
                var_6 = _id_C218( var_0 );

                if ( isdefined( var_6 ) )
                    var_4 = var_6;
                else if ( isdefined( self.run_override_weights ) )
        ...
        if ( !var_2 )
            var_7 = common_scripts\utility::ter_op( isdefined( self._id_D175 ), self._id_D175, self.moveplaybackrate );
        else
            var_7 = 1.0;

        var_8 = animscripts\stairs_utility::_id_B79B();
