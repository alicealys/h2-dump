...
snd_common_init()
{
    ...
    level.player._id_BB2B = "radiation_none";
    level._snd.context_overrides = [ [ "mute", 0 ], [ "predator", 0 ], [ "credits", 0 ], [ "slomo", 0 ], [ "deathsdoor", 0 ], [ "underwater", 0 ], [ "diveboat", 0 ], [ "interior_vehicle", 0 ], [ "wpn_int_med", 0 ], [ "wpn_int_sml", 0 ], [ "wpn_semi_open", 0 ], [ "bullet_metal_vehicle", 0 ], [ "bullet_whizby_glass", 0 ], [ "finale_handgun", 0 ] ];
...
register_common_snd_messages()
{
    ...
    soundscripts\_snd::snd_register_message( "friendly_fire_mission_failed", ::_id_AFE1 );
    soundscripts\_snd::snd_register_message( "end_mission_fade_to_black", ::_id_BB0F );
    soundscripts\_snd::snd_register_message( "specops_end_mission", ::_id_B893 );
    ...
    soundscripts\_snd::snd_register_message( "start_vehicle_shell_shock", ::_id_C8FE );
    soundscripts\_snd::snd_register_message( "aud_radiation_shellshock", ::_id_A991 );
    ...
    soundscripts\_snd::snd_register_message( "aud_start_ragtime_warfare", ::_id_D1E2 );
    soundscripts\_snd::snd_register_message( "aud_stop_ragtime_warfare", ::_id_B00E );
    soundscripts\_snd::snd_register_message( "aud_start_slowmo_cheat", ::aud_start_slowmo_cheat );
    soundscripts\_snd::snd_register_message( "aud_stop_slowmo_cheat", ::aud_stop_slowmo_cheat );
}
...
_id_AFE1()
{
    if ( !isdefined( level._id_B694 ) && common_scripts\utility::issp() )
    {
...
_id_B893()
{
    soundscripts\_audio_mix_manager::mm_add_submix( "so_end_mission_mix" );
}
...
_id_D17A()
{
    ...
    for (;;)
    {
        ...
        while ( level.player maps\_utility::isads() )
        {
            var_1 = level.player maps\_utility::_id_AB7A();

            if ( !var_0 && var_1 )
                soundscripts\_audio_mix_manager::mm_add_submix( "sniper_ads_holdbreath_mix" );
            else if ( var_0 && !var_1 )
                soundscripts\_audio_mix_manager::mm_clear_submix( "sniper_ads_holdbreath_mix" );
        ...
        if ( var_8 < var_1 )
        {
            ...
            if ( !isdefined( self.aud.mute_device_filter ) && isdefined( var_4 ) )
            {
                ...
                if ( !isdefined( self.aud.mute_device_enter ) )
                {
                    ...
                    self.aud.loop_ent setvolume( 1.0, 2 );
                }
            }
        }
        else if ( var_8 < var_6 && self.aud.mute_device_start == 1 )
        {
            ...
            if ( isdefined( self.aud.mute_device_filter ) )
            {
                ...
                if ( !isdefined( self.aud.mute_device_exit ) )
                {
                    ...
                    self.aud.loop_ent setvolume( 0.0, 2 );
                }
            }
        }
...
boost_land_player( var_0 )
{
    ...
    if ( var_3 == level.player )
    {
        if ( level._snd.boost_jump.is_jumping == 1 )
        {
            ...
            var_7 = _func_245( var_3.origin + ( 0.0, 0.0, 16.0 ), var_3.origin + ( 0.0, 0.0, -16.0 ), var_3 )["surfacetype"];
...
_id_D1E2()
{
    ...
    level.player thread common_scripts\utility::play_loop_sound_on_entity( "cheat_chaplin_music" );
    level.player thread common_scripts\utility::play_loop_sound_on_entity( "cheat_chaplin_projector_loop" );
}
...
{
    soundscripts\_audio_mix_manager::mm_add_submix( "slowmo_cheat_mix" );
    level.player _meth_8521();
    snd_enable_soundcontextoverride( "slomo" );
    level.player playsound( "scn_cheat_slomo_in" );
    level.player thread common_scripts\utility::play_loop_sound_on_entity( "scn_cheat_slomo_lp", undefined, 0.8, 0.8 );
}

aud_stop_slowmo_cheat()
{
    soundscripts\_audio_mix_manager::mm_clear_submix( "slowmo_cheat_mix" );
    level.player _meth_8522();
    snd_disable_soundcontextoverride( "slomo" );
    level.player common_scripts\utility::stop_loop_sound_on_entity( "scn_cheat_slomo_lp" );
    level.player playsound( "scn_cheat_slomo_out" );
}

_id_A991( var_0, var_1 )
{
    level.player endon( "death" );
    soundscripts\_audio_zone_manager::azm_set_filter_bypass( 1 );

    if ( level.player._id_BB2B != var_0 )
    {
        level.player._id_BB2B = var_0;

        switch ( level.player._id_BB2B )
        {
            case "radiation_high":
                _id_C948( "rad_shellshock_high", "item_geigercounter_high", "breathing_hurt" );
                break;
            case "radiation_med":
                _id_C948( "rad_shellshock_med", "item_geigercounter_med", "breathing_hurt" );
                break;
            case "radiation_low":
                _id_C948( "rad_shellshock_low", "item_geigercounter_med", "breathing_hurt" );
                break;
            case "radiation_none":
                _id_B1AB( 4.0, "breathing_better" );
                break;
        }
    }
}

_id_C948( var_0, var_1, var_2 )
{
    var_3 = 0.5;
...
snd_monitor_no_ammo_mix()
{
    ...
    for (;;)
    {
        ...
        if ( !var_0 && var_1 == 0 )
        {
            soundscripts\_audio_mix_manager::mm_add_submix( "no_ammo_mix" );
            var_0 = 1;
        }
        else if ( var_0 && var_1 > 0 )
        {
            soundscripts\_audio_mix_manager::mm_clear_submix( "no_ammo_mix" );
            var_0 = 0;
        }
