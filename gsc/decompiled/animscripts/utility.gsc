...
initialize( var_0 )
{
    ...
    if ( isdefined( self.node ) && ( self.node.type == "Conceal Prone" || self.node.type == "Conceal Crouch" || self.node.type == "Conceal Stand" ) )
    {
        ...
        self.a._id_CD14 = level.script == "favela";
    }
...
isincombat( var_0 )
{
    if ( isdefined( self.stairsstayincombat ) && self.stairsstayincombat && animscripts\stairs_utility::_id_D458() )
        return 1;

    var_1 = isincombatinternal( var_0 );

    if ( var_1 && animscripts\stairs_utility::_id_D458() )
...
_id_B572( var_0, var_1 )
{
    if ( !isdefined( anim._id_B274 ) )
        anim._id_B274 = [];

    anim._id_B274[var_0] = 1;
    level._effect["bloodpool_" + var_0] = var_1;
}
...
_id_B9E0( var_0 )
{
    if ( isdefined( self.a._id_C956 ) && self.a._id_C956 == self.a.runloopcount )
        return self.a._id_AD3B;

    var_1 = randomint( var_0.size );
    self.a._id_AD3B = var_0[var_1];
    self.a._id_C956 = self.a.runloopcount;
    return var_0[var_1];
}
...
{
    return isdefined( self.a.array[var_0] );
}
...
_id_AD5A( var_0, var_1 )
{
    var_2 = self.a.array[var_0].size - 1;
    var_3 = randomint( var_2 - 1 );

    if ( var_3 == var_1 )
        var_3 = var_2;

    return self.a.array[var_0][var_3];
}
...
aihasweapon( var_0 )
{
    if ( isdefined( self.weaponinfo ) && isdefined( self.weaponinfo[var_0] ) )
        return 1;
...
_id_BA45( var_0, var_1 )
{
    if ( !isdefined( level._id_C7D6 ) )
    {
        level._id_C7D6 = [];
        level._id_C7D6["allies"] = 0;
        level._id_C7D6["axis"] = 0;
        level._id_C7D6["neutral"] = 0;
    }
    ...
    if ( level._id_C7D6[var_0] != var_1 )
        level._id_C7D6[var_0] = var_1;
}
...
_id_BBC3()
{
    if ( isai( self ) && isdefined( self._id_C92A ) )
        return !self._id_C92A;
...
_id_BB4A()
{
    return !( isdefined( self.casual_killer ) && self.casual_killer );
}
...
_id_BF29()
{
    ...
    if ( self.animname == "meat" )
        return 0;

    if ( self.animname == "royce" )
        return 0;
...
should_play_specific_covercrouch_anim()
{
    ...
    {
        case "cover_crouch":
            foreach ( var_1 in lookupanim( "exposed_turn_crouch", "exposed2cover_turn_all" ) )
            {
        ...
        case "pain":
            foreach ( var_1 in lookupanim( "pain", "covercrouch_all" ) )
            {
            ...
            return 0;
        case "combat":
            foreach ( var_1 in lookupanim( "default_crouch", "cover2exposed_turn_all" ) )
            {
                if ( self getanimweight( var_1 ) > 0 && self getanimtime( var_1 ) < 0.5 )
                    return 1;
            }
