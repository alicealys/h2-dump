...
runonshootbehaviorend()
{
    ...
    if ( !isdefined( self._id_D330 ) || !self._id_D330 )
    {
        self.a.laseron = 0;
        animscripts\shared::updatelaserstatus();
    }
...
sniper_glint_behavior()
{
    ...
    for (;;)
    {
        if ( self.weapon == self.primaryweapon && animscripts\combat_utility::player_sees_my_scope() && ( !isdefined( self.isholdinggrenade ) || !self.isholdinggrenade ) )
        {
            if ( distancesquared( self.origin, self.enemy.origin ) > 65536 )
            {
                self notify( "abort_glint" );
                playfxontag( var_0, self, "tag_flash" );
                thread _id_CF70( var_0 );
            }
...
_id_CF70( var_0 )
{
    ...
    self endon( "death" );
    common_scripts\utility::waittill_any( "began_to_switch_to_sidearm", "killanimscript" );
