...
_autosave_game_now( var_0 )
{
    ...
    if ( issaverecentlyloaded() )
    {
        ...
        level._id_CC1F = level.lastautosavetime;
        return 0;
    }
    ...
    if ( try_to_autosave_now() )
    {
        ...
        if ( !isdefined( var_0 ) )
            thread _id_D36F::_id_C5AF();

        commitsave( var_5 );
        level._id_CDDD = gettime();
        setdvar( "ui_grenade_death", "0" );
    }
...
autosaveplayercheck( var_0 )
{
    ...
    if ( maps\_utility::player_is_owner_of_live_grenade() )
    {
        autosaveprint( "autosave failed:player is owner of a live grenade" );
        return 0;
    }

    if ( isdefined( self.is_controlling_uav ) && self.is_controlling_uav == 1 )
    {
        autosaveprint( "autosave failed:player is in UAV" );
        return 0;
    }
