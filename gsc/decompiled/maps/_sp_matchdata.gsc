...
register_death( var_0, var_1 )
{
    ...
    maps\_utility::_id_D403();
    var_3 = level.player getplayerdata( common_scripts\utility::_id_AC0E(), "career", "deaths_total" );

    if ( isdefined( var_3 ) )
        level.player setplayerdata( common_scripts\utility::_id_AC0E(), "career", "deaths_total", var_3 + 1 );

    var_4 = level.missionsettings maps\_endmission::getlevelindex( level.script );

    if ( isdefined( var_4 ) )
    {
        var_5 = level.player getplayerdata( common_scripts\utility::_id_AC0E(), "career", "campaign", level.gameskill, "levels", level.script, "current_playtrough_deaths" );

        if ( isdefined( var_5 ) )
        {
            var_5++;
            level.player setplayerdata( common_scripts\utility::_id_AC0E(), "career", "deaths_total", var_5 );
        }
    }
}
