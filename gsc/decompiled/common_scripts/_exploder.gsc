...
setupexploders()
{
    ...
    for ( var_2 = 0; var_2 < var_6.size; var_2++ )
    {
        ...
        var_4 common_scripts\_createfx::_id_C30A();
    }
...
_id_B788()
{
    ...
    if ( isdefined( level.createfxexploders ) )
    {
        foreach ( var_2 in level.createfxexploders )
            var_0 = common_scripts\utility::array_combine( var_0, var_2 );
    }
    else
    {
        foreach ( var_5 in level.createfxent )
        {
...
_id_C728()
{
    ...
    if ( isdefined( self._id_AD5C ) )
    {
        var_2 = common_scripts\utility::get_target_ent( self._id_AD5C );

        if ( isdefined( var_2 ) )
            var_1 = var_2.origin;
    }
    else if ( isdefined( self._id_CC2D ) )
        var_1 = self._id_CC2D;
...
_id_CD78()
{
    ...
    if ( !var_2 && isdefined( self.v["delay"] ) && self.v["delay"] > 0.0 )
    {
        var_3 = self.v["delay"];
        wait( var_3 );
    }
    ...
    {
        if ( !isdefined( var_0._id_A801 ) )
        {
            var_4 = self.model _id_C728();
            var_0._id_A801 = var_4;
        }

        if ( var_2 )
        {
            var_0._id_A801 hide();
            var_0._id_A801 notsolid();
        }
        else
        {
            var_0._id_A801 show();
            var_0._id_A801 solid();
        }
    }
    ...
    if ( isdefined( self.v["delay_post"] ) && self.v["delay_post"] > 0.0 )
    {
        var_5 = self.v["delay_post"];
        wait( var_5 );
        var_3 += var_5;
    }
    ...
    if ( level.createfx_enabled && var_1 )
    {
        var_0 endon( "death" );
        var_0 endon( "play_exploder_anim" );
        var_6 = _id_A926();

        if ( isdefined( level._id_AA53 ) )
        {
            var_7 = self.v["exploder"] + "";

            if ( isdefined( level._id_AA53[var_7] ) )
            {
                var_6 = max( level._id_AA53[var_7], var_6 );
                var_6 -= var_3;
            }
        }
        ...
        {
            var_0._id_A801 delete();
            var_0._id_A801 = undefined;
        }
    }
}
...
exploder_rumble()
{
    ...
    exploder_delay();
    level.player playrumblelooponentity( self.v["rumble"] );
}
...
_id_B946( var_0 )
{
    ...
    {
        var_1 = self.v["tintcolor"];

        if ( !isdefined( var_1 ) )
            var_1 = ( 1, 1, 1 );

        var_2 = self.v["tintalpha"];

        if ( !isdefined( var_2 ) )
            var_2 = 1;

        var_3 = self.v["emissive"];

        if ( !isdefined( var_3 ) )
            var_3 = 1;

        var_4 = self.v["sizescale"];

        if ( !isdefined( var_4 ) )
            var_4 = 1;

        self.looper = spawnfx( level._effect[self.v["fxid"]], self.v["origin"], self.v["forward"], self.v["up"], var_1, var_2, var_3, var_4 );
    }
    else
        self.looper = spawnfx( common_scripts\utility::getfx( self.v["fxid"] ), self.v["origin"], self.v["forward"], self.v["up"] );

    var_5 = isdefined( self.v["kill_on_delete"] ) && self.v["kill_on_delete"] || level.createfx_enabled;
