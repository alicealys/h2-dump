...
initfx()
{
    ...
    for ( var_1 = 0; var_1 < level.createfxent.size; var_1++ )
    {
        ...
        var_2 common_scripts\_createfx::set_forward_and_up_vectors();
        var_2 common_scripts\_createfx::_id_C30A();
        ...
        switch ( var_2.v["type"] )
        {
            ...
            case "soundfx_dynamic":
                var_2 thread create_dynamicambience();
                break;
        }

        if ( isdefined( var_2.v["stop_on_exploder"] ) )
            var_2 thread common_scripts\_createfx::_id_A9A8();

        var_2 _id_CDD1( var_0 );
    }

    check_createfx_limit( var_0 );
}
...
create_dynamicambience()
{
    ...
    if ( isdefined( self.dambinfostruct ) )
        level.player _meth_848D( self.dambinfostruct.unique_id );

    self.dambinfostruct = spawnstruct();
    self.dambinfostruct common_scripts\utility::assign_unique_id();
    level.player _meth_848C( self.v["ambiencename"], self.v["origin"], self.v["dynamic_distance"], self.dambinfostruct.unique_id );
    return;
}
...
create_triggerfx()
{
    ...
    if ( isdefined( self.v["tintcolor"] ) || isdefined( self.v["tintalpha"] ) || isdefined( self.v["emissive"] ) || isdefined( self.v["sizescale"] ) )
    {
        ...
        var_2 = self.v["emissive"];

        if ( !isdefined( var_2 ) )
            var_2 = 1;

        var_3 = self.v["sizescale"];

        if ( !isdefined( var_3 ) )
            var_3 = 1;

        self.looper = spawnfx( level._effect[self.v["fxid"]], self.v["origin"], self.v["forward"], self.v["up"], var_0, var_1, var_2, var_3 );
    }
    else
        self.looper = spawnfx( level._effect[self.v["fxid"]], self.v["origin"], self.v["forward"], self.v["up"] );

    triggerfx( self.looper, self.v["delay"] );
    var_4 = isdefined( self.v["kill_on_delete"] ) && self.v["kill_on_delete"] || level.createfx_enabled;

    if ( var_4 )
        _func_15E( self.looper, 1 );
...
_id_BF39( var_0, var_1, var_2, var_3 )
{
    if ( !isdefined( var_2 ) )
        var_2 = 1;
    ...
    if ( var_2 && isdefined( var_1._id_C296 ) )
        var_0[var_1._id_C296] = 1;

    if ( var_3 && isdefined( var_1._id_C173 ) )
        var_0[var_1._id_C173] = 1;

    return var_0;
}

_id_C85D( var_0, var_1, var_2 )
{
    var_3 = [];
    ...
    if ( isarray( var_0 ) )
    {
        foreach ( var_5 in var_0 )
            var_3 = _id_BF39( var_3, var_5, var_1, var_2 );
    }
    else
        var_3 = _id_BF39( var_3, var_0, var_1, var_2 );
...
_id_C9D2()
{
    ...
    var_3 = level._id_C179;

    if ( isdefined( level._id_A9A7 ) )
        var_3[var_3.size] = level._id_A9A7;

    var_4 = _id_C85D( var_3 );

    foreach ( var_7, var_6 in var_4 )
        thread _id_C1EB( var_7 );
}
...
_id_C1EB( var_0 )
{
    ...
    for (;;)
    {
        var_1 = common_scripts\utility::flag( var_0 );
        level waittill( var_0 );
        var_2 = common_scripts\utility::flag( var_0 );

        if ( var_2 == var_1 )
            continue;

        level thread _id_C626();
    }
}
...
_id_C626()
{
    if ( isdefined( level._id_B3E0 ) )
    {
        if ( level._id_B3E0 == gettime() )
            return;
    ...
    level._id_B3E0 = gettime();
    waittillframeend;
    var_0 = 0;
    var_1 = 50;

    if ( isdefined( level._id_B065 ) )
        var_1 = level._id_B065;
    ...
    {
        if ( !isdefined( var_6 ) || !isdefined( var_6.v["fx_volume_groups"] ) )
            continue;

        var_7 = var_6 _id_C3ED();
        var_8 = isdefined( var_6.looper );
    ...
    {
        var_6 common_scripts\_createfx::stop_fx_looper();
        var_0 = fx_volume_group_throttle_update( var_0, var_1 );
    }
    ...
    {
        var_6 common_scripts\_createfx::restart_fx_looper();

        if ( isdefined( var_6.looper ) && ( !isdefined( var_6.v["kill_on_delete"] ) || isdefined( var_6.v["kill_on_delete"] ) && !var_6.v["kill_on_delete"] ) )
        {
            var_4[var_4.size] = var_6.looper;
            _func_15E( var_6.looper, 1 );
        }
...
_id_C3ED()
{
    ...
    if ( !isdefined( self.v["fx_volume_groups"] ) )
        return 1;

    var_0 = [];
    var_1 = [];

    if ( self.v["fx_volume_groups"].size != 0 )
    {
        var_2 = [];

        foreach ( var_4 in self.v["fx_volume_groups"] )
        {
            var_5 = level._id_C179[var_4];

            if ( isdefined( var_5 ) )
                var_2[var_2.size] = var_5;
        }

        var_0 = _id_C85D( var_2, 1, 0 );
        var_1 = _id_C85D( var_2, 0, 1 );
    }
    else if ( isdefined( level._id_A9A7 ) )
    {
        var_0 = _id_BF39( var_0, level._id_A9A7, 1, 0 );
        var_1 = _id_BF39( var_1, level._id_A9A7, 0, 1 );
    }
