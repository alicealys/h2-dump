...
LUI.MenuBuilder.registerType( "astronautHelmetHudDef", function ()
	...
	self.tryInit = function ()
		if self.isInit ~= nil and not self.isInit then
			...
			local self = LUI.UIImage.new( f5_local2 )
			self.rightBlur = self
			self:addElement( self )
			...
			self.vignette.topLeft = self
			self.vignette:addElement( self )
			...
			local self = LUI.UIImage.new( f5_local8 )
			self.vignette.topRight = self
			self.vignette:addElement( self )
			...
			self:setup3SliceRatio( 1487.2, 0.48 )
			self.vignette.bottom = self
			self.vignette:addElement( self )
			...
			local self = LUI.UIImage.new( f5_local12 )
			self:setup3SliceRatio( 654.67, 0.48 )
			self.vignette.bottomBlur = self
			self.vignette:addElement( self )
			local f5_local14 = self
			local f5_local15 = LUI.UITimer.new
			local f5_local16 = 1
			local f5_local17 = "update_astronaut_overlay"
			local f5_local18 = nil
			local f5_local19, f5_local20 = false
			f5_local14.updateTimer = f5_local15( f5_local16, f5_local17, f5_local18, f5_local19, f5_local20, false, false, true )
